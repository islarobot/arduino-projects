...
int calcula_diferencia(){ 
  ...
  if((sensores[FL1] > valor_neutro + P_MUERTO)&&(sensores[FL2] > valor_neutro + P_MUERTO)){
    ...
    if((valor > P_MUERTO) || (valor < -P_MUERTO)){// Si el valor cambia por encima de P_MUERTO el robot cambiará de posición según la diferencia de un sensor a otros      
      ...
      if (valor > 0) {valor= GIRO_LINEA;direccion_anterior=0;}  // Giramos a la derecha para entrar de nuevo en la línea
      if(valor < 0){valor = -GIRO_LINEA;direccion_anterior=1;}  // Giramos a la izquierda para entrar de nuevo en la línea
    }
